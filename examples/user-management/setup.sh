...
echo "Setting up user management transaction examples..."
...
# Verify setup
docker exec -i cassandra-accord ./bin/cqlsh -e "SELECT email, country, city FROM users.user;"

echo ""
echo "Setup complete! You can now run user management transactions using:"
